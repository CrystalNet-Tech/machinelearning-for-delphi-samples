unit SrEntireDetectionConsoleApp;

interface

uses MLContextMgr, SrEntireDetection_Models, MLData, MLCore;

type
  TSrEntireDetectionConsoleApp = class
  private
    class var mlContext: IMLContextManager;
    class procedure DetectAnomaly(DataView: IMLDataView);
  public
    class procedure Run;
  end;
...
const
  ...
  DatasetRelativePath = BaseDatasetsRelativePath + '\phone-calls.csv';


{ TSrEntireDetectionConsoleApp }


class procedure TSrEntireDetectionConsoleApp.Run;
const
  size = 36;
var
  DatasetPath,
...
begin
  ...
  var dataView := mlContext.Data.LoadFromTextFile<TPhoneCallsData>(DatasetPath, true, ',');
...
class procedure TSrEntireDetectionConsoleApp.DetectAnomaly(DataView: IMLDataView);
begin
  TConsole.NClass.WriteLine('===============Detect anomalies in pattern===============');

  //STEP 1: Specify the input column and output column names.
  var inputColumnName: string := 'value';
  var outputColumnName: string := 'Prediction';
  ...
  TConsole.NClass.WriteLine('Period of the series is: {0}.', period);
